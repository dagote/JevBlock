...
Extension version is `extension/manifest.json` (**0.1.3**). Server version is **0.3.2**. Page-judge timeout is 15 minutes with background single-flight; server scores ad-like candidates first and returns remapped kinds within a budget. Force-hide cheats stay off. Block OFF = classify-only decision log.
...
2. Load unpacked `extension/` and confirm the card says **0.1.3**. Reload if it still says 0.1.2 or older.
...
4. Open a page, reload so 0.1.3 attaches, click **Judge this tab** with Block **off** first. Review should list candidates with kind+noul (Advertisement/ad.com should be `ad`, not `nav_chrome` or `judge_error`). Then enable Block and ranks to remove.
...
- Reload **0.1.3** and restart adgate **0.3.2**. Extreme force-hide cheats stay off for product retests. A unit test is not a live Chrome pass.
