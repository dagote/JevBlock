...
Extension version is `extension/manifest.json` (**0.1.2**). Server version is **0.3.1**. Kind choice asks clear ad vs nav_chrome criteria; element state includes discover/hosts. Force-hide cheats stay off. Block OFF = classify-only decision log.
...
2. Load unpacked `extension/` and confirm the card says **0.1.2**. Reload if it still says 0.1.1 or older.
3. Set the server URL. Confirm `GET /health` reports **0.3.1**. Enable **Block**. Configure hide ranks (ad/promo on by default). Leave Extreme force-hide cheats **off**.
4. Open a page, reload so 0.1.2 attaches, click **Judge this tab** with Block **off** first. Review should list candidates with kind+noul (Advertisement/ad.com should be `ad`, not `nav_chrome`). Then enable Block and ranks to remove.
...
- Reload **0.1.2** and restart adgate **0.3.1**. Extreme force-hide cheats stay off for product retests. A unit test is not a live Chrome pass.
