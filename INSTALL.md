...
The unpacked extension version in this repo is `extension/manifest.json` (currently **0.0.7**). The 0.3.3 notes below describe an older Chrome-vs-disk mismatch; they are not the current package version. Server **0.2.4** is the matching adgate build. Reload the unpacked extension before a live Extreme retest.
